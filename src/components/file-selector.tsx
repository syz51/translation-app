...
import { Film, FolderOutput, Upload } from 'lucide-react'
  ...
  return (
    <section
      className={`border-2 border-dashed p-8 transition-colors ${
        isDragging
          ? 'rounded-[1.9rem] border-[var(--app-accent)] bg-[linear-gradient(135deg,rgba(247,179,43,0.14),rgba(247,179,43,0.04))]'
    ...
    >
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_260px] lg:items-center">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`rounded-3xl border p-4 transition-colors ${
                isDragging
                  ? 'border-[#f7b32b]/40 bg-[#f7b32b]/10 text-[#f7b32b]'
            ...
            <div>
              ...
              <h3 className="display-type mt-1 text-3xl text-white">
                {isDragging ? 'Drop video files now' : 'Load a video batch'}
              </h3>
            </div>
          </div>

          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/[0.68] sm:text-base">
            Drag files straight into the workspace or browse manually. The first
            folder you choose becomes the default output location if nothing is
            set yet.
          </p>
        ...
        <div className="grid gap-3">
          ...
          <div className="rounded-3xl border border-white/10 bg-black/[0.18] p-4">
            <div className="flex items-center gap-3">
              <FolderOutput className="h-5 w-5 text-[var(--app-highlight)]" />
